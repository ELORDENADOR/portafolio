           ...
           <div id='configuracinImagenTexto'>
              ...
              <Formik
               ...
               onSubmit={async values => {
                   const response = await fetch(`https://api.unsplash.com/search/photos?per_page=${mas}&query=${values.search}`,{
                       headers:{
                           'Authorization':'Client-ID RY68GpT6qCvAbSSuIJS1n8POvBIzml0iz_PWZK8atzY'
                   }})
              ...
              >        
                <Form>
...

                   {
                       ...
                       (
                        <div>
                            <button id='botonBuscarApi1'onClick={() => activarBuscarMasImagenes()} >Buscar</button>
                            <button id='botonActivarImg1' onClick={() => masImagenes()} >Mas</button>
                       </div>
                       ) 
                       :
                       (
...
#botonBuscarApi1,#botonBuscarApi2{
    background: tomato; 
}
